...
test("Type flows", async ({ page }) => {
  ...
  // Edit
  await page.getByRole("row", { name: /Test Type for types/ }).getByRole("button", { name: "Edit" }).click();
  await page.getByLabel("Name:").fill("Test Type Edited");
  await page.getByRole("button", { name: "Save" }).click();
  await expect(page.getByText("Test Type Edited")).toBeVisible();

  // Delete
  await page.getByRole("row", { name: /Test Type Edited/ }).getByRole("button", { name: "X" }).click();
